...
```bash
go install github.com/skills-lock/skil-lock/cmd/skil-lock@v0.2.4
```
...
curl -sL https://github.com/skills-lock/skil-lock/releases/download/v0.2.4/skil-lock_0.2.4_${OS}_${ARCH}.tar.gz | tar -xz
...
```powershell
$url  = "https://github.com/skills-lock/skil-lock/releases/download/v0.2.4/skil-lock_0.2.4_windows_amd64.zip"
...
- CLI: `v0.2.4` - every SARIF run now declares what it actually analysed. Previously, in warn mode (the default when no `.skil-lock.yaml` exists), a `SKILL.md` that failed to parse dropped out of the scan with the error going only to stderr — so a run that silently skipped a skill produced a report identical to one where everything parsed. Runs now carry `run.properties.completeness` (including on clean runs, so silence is a claim rather than an absence), a `warning` notification naming each unanalysed skill, and a proper failure channel (`executionSuccessful: false`) when the scan itself cannot run. See [SPEC §14.4](SPEC.md#144-run-completeness). v0.2.3 added `target.digest` (the skill's SHA-256 content hash, in `artifacts[].hashes`), so each drift finding is pinned to the exact SKILL.md content it was raised against and self-invalidates when the file changes. v0.2.2 added the OWASP Agentic Skills Top 10 (AST10) taxonomy (per-rule + per-finding AST risk IDs) so Code Scanning alerts map to AST01-AST10. v0.2.1 added exhaustive sibling-file digests (every file a skill ships is hashed, not just `scripts/` and `resources/`) and PR-scoped approvals (a reverted-then-reintroduced delta re-blocks instead of riding a stale approval). v0.2.0 added per-bundled-script content digests, cosign keyless signing + SLSA provenance + SBOM on releases, and a documented detection boundary; v0.1.2 was a renderer typography fix; v0.1.1 added SARIF output + multi-platform release binaries.
- GitHub Action: [`skil-lock-action@v0.2.1`](https://github.com/skills-lock/skil-lock-action/releases/tag/v0.2.1) - unchanged; set `pin-binary: v0.2.4` to run the latest CLI.
